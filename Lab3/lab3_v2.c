...
int main(int argc, char *argv[])
{
  ...
  if(compression)
  ...
  else
    {
      ...
      //Intialize the dictionary------------------------------------------------
      //printf("here");
      unsigned char *X, Y, Z, *XY, *XZ, **dictionary;
      dictionary = (unsigned char **)calloc(MAX_LENGTH,sizeof(unsigned char *));
      int *length;
      unsigned short int dictionaryLen = 0;
          ...
          if(C <= dictionaryLen)
            {
              ...
              Y = dictionary[C][0];
              //printf("%c",Y);
              XY = (unsigned char *)calloc(length[P]+ 1,sizeof(unsigned char));
              memcpy(XY,X,length[P]);
              memcpy((XY + length[P]),&Y,1);
              //add XY to the dictionary
              dictionaryLen++;
              dictionary[dictionaryLen] = (unsigned char *)calloc((length[P] + 1),sizeof(unsigned char));
              memcpy(dictionary[dictionaryLen],XY,(length[P]+1));
          ...
          else
            {
              ...
              memcpy(X,dictionary[P],length[P]);
              Z = dictionary[P][0];
              XZ = (unsigned char *)calloc(length[P]+ 1,sizeof(unsigned char));
              memcpy(XY,X,length[P]);
              memcpy((XZ + length[P]),&Z,1);
              //add XY to the dictionary and output XZ
              fwrite(XZ,sizeof(unsigned char),length[P]+1,outputFile);
              dictionaryLen++;
              dictionary[dictionaryLen] = (unsigned char *)calloc((length[P] + 1),sizeof(unsigned char));
              memcpy(dictionary[dictionaryLen],XZ,(length[P]+1));
              length[dictionaryLen] = length[P] + 1;
              //free(X);
              free(XZ);
            }
